...
Источники для MVRV/NUPL/SOPR (по умолчанию — proxy §8.10 из ряда plan01; остальное дозаполняет пропуски):
1. `build_market_history` (FreeCryptoAPI + SQLite) — те же формулы §8.10 в `coingecko_onchain`, флаг `USE_COINGECKO_ONCHAIN`
