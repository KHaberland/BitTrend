...
Опционально: в `requirements.txt` закомментирован **ccxt** (реальное исполнение сделок — отдельный апгрейд по плану).
...
4. **Execute Part 1/2/3** — MVP: логирование объёма сделки (реальные ордеры не исполняются).
